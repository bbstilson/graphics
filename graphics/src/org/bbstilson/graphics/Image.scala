...
case class Image(width: Int, height: Int, fileName: Option[String] = None) {
  private val imageBuffer = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB)
  ...
  private val outputFile = {
    ...
    new File(List(desktopPath, finalFileName, ".jpg").mkString)
  }

  /**
    * Generates a full jpg image by providing every pixel (x, y) to a generator function
    * which returns a Color that the pixel should be.
    *
    * @param generator
    */
  ...
  /**
    ...
    * @param color
    */
  def writePixel(x: Int, y: Int, color: Color): Unit = imageBuffer.setRGB(x, y, color.getRGB())
